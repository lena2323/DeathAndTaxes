...
export default function InputFormIncomeYearly(props) {
  ...
  return (
    <div className='relative lg:px-8 font-philosopher flex-col items-center justify-center text-center lg:rounded-3xl   lg:max-w-xl lg:mx-auto lg:px-20'>
      <div	class="lg:absolute inset-0 lg:bg-white  shadow-lg transform -skew-y-6 sm:skew-y-0 lg:-rotate-6 lg:rounded-3xl bg-gradient-to-r from-lime-300 to-lime-600  ">  
      </div>
      ...
      <div className='flex my-4 justify-between items-center text-left '>
        ...
        <input className=' w-20 text-sm text-gray-800 bg-white border rounded py-2 px-3 hover:border-lime-300 focus:border-lime-400 shadow-md outline-0 text-right select-none' type="number" min={1}  max={1000000000000} placeholder="0" required onChange={e => setGrossIncomeYearly(e.target.value)}></input>
      </div>

      <div className='flex my-4 justify-between text-left items-center'>
        <label>How many children do you have?</label>
        <input className=' w-20 text-sm text-gray-800 bg-white border rounded py-2 px-3 hover:border-lime-300 focus:border-lime-400 shadow-md outline-0 text-right select-none' type="number"  min={0} max={150} placeholder="0" required onChange={e => setChildrenNumber(e.target.value)}></input>
      </div>

      <div className='flex my-4 justify-between text-left items-center'>
        <label>How many properties do you own?</label>
        <input className=' w-20 text-sm text-gray-800 bg-white border rounded py-2 px-3  hover:border-lime-300 focus:border-lime-400  shadow-md  outline-0 text-right select-none' type="number" required min={0} max={100} placeholder="0" onChange={e => setPropertyOwnership(e.target.value)} ></input>
      </div>
      ...
      <div className='flex font-philosopher my-4 justify-between text-left gap-5 items-center'>
        ...
        <select className='text-gray-800 bg-white border rounded py-3 px-3  hover:border-lime-300 focus:border-lime-400 shadow-md outline-0 select-none' name='MaritalStatus' required onChange={e => setMaritalStatus(e.target.value)}>
